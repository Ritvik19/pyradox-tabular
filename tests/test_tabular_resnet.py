...
def test_mixed_data_types():
    ...
    model_config = TabularResNetConfig(num_outputs=1, out_activation=None, hidden_units=[64, 64])
    ...
    model = TabularResNet.from_config(data_config, model_config, name="mixed_data_types")
...
def test_all_num_type():
    ...
    model = TabularResNet.from_config(data_config, model_config, name="all_num_type")
...
def test_all_cat_type():
    ...
    model = TabularResNet.from_config(data_config, model_config, name="all_cat_type")
    model.compile(optimizer="adam", loss="binary_crossentropy")
    model.fit(data_train, validation_data=data_valid)
