...
def test_mixed_data_types():
    ...
    model = TabTransformer.from_config(data_config, model_config, name="mixed_data_types")
...
def test_all_num_type():
    ...
    model_config = TabTransformerConfig(
        num_outputs=1, out_activation="sigmoid", num_transformer_blocks=3, num_heads=4, mlp_hidden_units_factors=[2, 1]
    )
    data_train = DataLoader.from_df(x_train, y_train, batch_size=1024)
    data_valid = DataLoader.from_df(x_valid, y_valid, batch_size=1024)

    model = TabTransformer.from_config(data_config, model_config, name="all_num_type")
...
def test_all_cat_type():
    ...
    model = TabTransformer.from_config(data_config, model_config, name="all_cat_type")
    model.compile(optimizer="adam", loss="binary_crossentropy")
    model.fit(data_train, validation_data=data_valid)
